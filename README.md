...
## Automatische betaling (Mollie + e-mail)
De webshop kan automatisch betalen via Mollie (iDEAL, Bancontact, creditcard,
PayPal), met een bestelmail naar jou en een bevestigingsmail naar de klant.
De site blijft op GitHub Pages staan; er komt één klein stukje op Cloudflare bij.
De volledige stap-voor-stap uitleg staat in **`cloudflare-worker/README-betaling.md`**.
Kort: Worker plaatsen → je Mollie- en Resend-sleutel invullen → je Worker-URL
zetten in `PAY_ENDPOINT` boven in `assets/js/app.js`. Het vaste verzendbedrag
(`SHIPPING`) staat in `assets/js/app.js` én in de Worker — houd die gelijk.
