  ...
  function renderCheckout() {
    ...
    if (form && !form._bound) {
      ...
      form.addEventListener("submit", function (ev) {
        ...
        // Geen betaal-endpoint ingesteld -> demo-bevestiging tonen
        if (!PAY_ENDPOINT) {
        ...
        // Echte betaling: bestelling naar de Worker, dan door naar Mollie
        var g = function (n) { var el = form.querySelector('[name="' + n + '"]'); return el ? el.value.trim() : ""; };
        var order = {
          items: readCart().map(function (i) { return { slug: i.slug, qty: i.qty }; }),
          customer: {
        ...
        if (!order.items.length) { location.href = BASE + "winkelwagen/"; return; }
        ...
        fetch(PAY_ENDPOINT.replace(/\/$/, "") + "/create-payment", {
          ...
          .then(function (d) {
            if (d && d.checkoutUrl) {
              ...
              throw new Error((d && d.error) || "Onbekende fout");
            }
          })
          .catch(function (err) {
            if (btn) { btn.disabled = false; btn.textContent = btn._t; }
            var note = form.querySelector("[data-pay-error]");
            if (note) { note.hidden = false; note.textContent = "Er ging iets mis bij het starten van de betaling. Probeer het opnieuw. (" + err.message + ")"; }
            else { alert("Er ging iets mis bij het starten van de betaling. Probeer het opnieuw."); }
          });
      });
    }
  }
