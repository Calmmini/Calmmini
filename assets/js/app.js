    ...
    if (form && !form._bound) {
      ...
      form.addEventListener("submit", function (ev) {
        ...
        function showThanks() {
          ...
          var wrap = $("[data-checkout-wrap]");
          if (wrap) wrap.hidden = true;
          if (ok) ok.hidden = false;
          window.scrollTo({ top: 0, behavior: "smooth" });
        }

        // Bestelregels netjes samenvatten voor de e-mail
        var lines = cart.map(function (i) {
          var p = PMAP[i.slug] || {};
          return i.qty + "x  " + (p.name || i.slug) + "  (" + eur((p.price || 0)) + " p/st)";
        }).join("\n");
        var sub = cartTotal();
        var naam = (g("voornaam") + " " + g("achternaam")).trim();
        var message =
          "NIEUWE BESTELLING\n\n" + lines +
          "\n\nSubtotaal: " + eur(sub) +
          "\n(verzendkosten worden door CALMMINI bevestigd)\n\n" +
          "KLANT\n" + naam + "\n" + g("email") +
          "\n" + g("straat") + " " + g("huisnummer") +
          "\n" + g("postcode") + " " + g("plaats") + "\n" + g("land");

        // Geen formuliersleutel ingesteld -> demo (toon bevestiging, verstuur niets)
        if (!FORM_KEY) { showThanks(); return; }
        ...
        fetch(FORM_ENDPOINT, {
          ...
          body: JSON.stringify({
            access_key: FORM_KEY,
            subject: "Nieuwe bestelling — CALMMINI",
            ...
            bestelling: lines,
            subtotaal: eur(sub),
            adres: g("straat") + " " + g("huisnummer") + ", " + g("postcode") + " " + g("plaats") + ", " + g("land"),
            message: message,
          }),
        })
          .then(function (r) { return r.json(); })
          .then(function (d) {
            if (d && d.success) { showThanks(); }
            else { throw new Error((d && d.message) || "Onbekende fout"); }
          })
          .catch(function (err) {
            if (btn) { btn.disabled = false; btn.textContent = btn._t; }
            var note = form.querySelector("[data-pay-error]");
            var msg = "Er ging iets mis bij het versturen van je bestelling. Probeer het opnieuw of mail ons rechtstreeks.";
