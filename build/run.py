...
## Productfoto's
Alle producten hebben een echte productfoto (achtergronden zijn gelijkgetrokken
naar één warme cream-tint). Wil je een foto vervangen? Zet een nieuw bestand met
dezelfde naam in `assets/img/products/` — aanbevolen: staand, ~900×1125 px, `.jpg`.
...
echte productfoto's) en de losse `assets/img/*.jpg` (hero.jpg e.d.).

## Nog in te vullen
De productgegevens (ingrediënten/basis, gebruik, waar goed voor, "goed om te
weten") en de prijzen staan ingevuld. Alleen deze zaken staan nog open:
- **Reviews, keurmerken, certificeringen, levertijden** — bewust niet verzonnen.
- **Juridische teksten** (privacy) — placeholder; voorwaarden zijn wél ingevuld.
- **Online betalen (Mollie)** — de webshop rekent online af via Mollie (iDEAL,
  Bancontact, creditcard) met **vaste verzendkosten € 8,00**. Bij een geslaagde
  betaling komt de bestelling per e-mail binnen op info@calmmini.nl. Instellen:
  zie `cloudflare-worker/README-betaling.md` en vul je Worker-URL in bij
  `PAY_ENDPOINT` boven in `assets/js/app.js`.
- **Contactformulier** — komt per e-mail binnen via Web3Forms; vul je Access Key
  in bij `FORM_KEY` boven in `assets/js/app.js`.
